...
type Props = {
  id: string;
  productDetailData: Partial<SaleProductFormData<ProductFormData<string>>>;
};

export default function ProductDetailInfo({ id, productDetailData }: Props) {
  const { openModal } = useModalContext();

  const images = useMemo(
    () => productDetailData.products?.map((d) => d.images).flat() ?? [],
    [productDetailData]
  );
